...
class ContextTest {
    ...
    void testSaveFile() {
        ...
        try {
            retFile = assertInstanceOf(File.class, handler.saveFile(pane, f));
        } catch (IOException e1) {
            e1.printStackTrace();
        }

        try {
            assertEquals(retFile, handler.loadFile(pane, retFile));
        } catch (IOException e1) {
            e1.printStackTrace();
        }
    }
    ...
    void testLoadFile(){
        ...
        try{
            handler.saveFile(pane, f);
        }catch (IOException e1){
            e1.printStackTrace();
        }

        try {
            assertInstanceOf(File.class, handler.loadFile(pane, f));
        } catch (IOException e1) {
            e1.printStackTrace();
        }
    }
